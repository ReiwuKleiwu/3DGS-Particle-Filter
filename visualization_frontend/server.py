...
from visualization_frontend.reset_command_store import PendingResetCommandStore
...
class VisualizationRequestHandler(BaseHTTPRequestHandler):
    ...
    def _serve_static_file(self) -> None:
        ...
        if not str(target_path).startswith(str(WEB_ROOT.resolve())) or not target_path.is_file():
            self._write_json(HTTPStatus.NOT_FOUND, {"detail": "Not found"})
            return
