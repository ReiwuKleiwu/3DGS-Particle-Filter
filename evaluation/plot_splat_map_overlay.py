...
from evaluation.plot_replay_paths import parse_map_yaml, world_to_image
...
def splat_xy_to_map_xy(splat_xy: np.ndarray, *, x: float, y: float, scale: float, yaw_degrees: float) -> np.ndarray:
    if scale == 0.0:
        raise ValueError("splat_map_scale must not be zero")
    yaw = math.radians(yaw_degrees)
    cosine = math.cos(yaw)
    sine = math.sin(yaw)
    rotation_inverse = np.array([[cosine, sine], [-sine, cosine]], dtype=np.float64)
    translated = splat_xy - np.array([x, y], dtype=np.float64)
    return (rotation_inverse @ translated.T).T / scale
...
def main() -> None:
    ...
    map_xy = splat_xy_to_map_xy(
        splat_xy,
        x=settings.renderer.splat_map_x,
        y=settings.renderer.splat_map_y,
        scale=settings.renderer.splat_map_scale,
        yaw_degrees=settings.renderer.splat_map_yaw_degrees,
    )
    image_xy = np.array(
        [
    ...
    ax.set_title(
        ...
        f"x={settings.renderer.splat_map_x:.4f}, y={settings.renderer.splat_map_y:.4f}, "
        f"scale={settings.renderer.splat_map_scale:.4f}, yaw={settings.renderer.splat_map_yaw_degrees:.3f} deg",
        fontsize=9,
    )
