...
from evaluation.plots.plot_replay_paths import parse_map_yaml, read_csv, world_to_image
from evaluation.plots.plot_style import GT_COLOR, PARTICLE_COLOR, PF_ALT_COLOR
...
def scenario_path_counts(rows: list[dict]) -> dict[str, set[str]]:
...
def particle_plot_title(frame_row: dict | None, *, run_id: str, frame_index: int, scenario_paths: dict[str, set[str]]) -> str:
    if frame_row is None:
        return f"{run_id}\nframe={frame_index}"

    scenario_id = frame_row.get("scenario_id", "")
    path_id = frame_row.get("path_id", "")
    mode = frame_row.get("localization_mode", "")
    splat_id = frame_row.get("splat_id", "")
    seed = frame_row.get("seed", "")
    parts = [part for part in (scenario_id, splat_id, mode) if part]
    if scenario_id and len(scenario_paths.get(scenario_id, set())) > 1 and path_id:
        parts.insert(1, path_id)
    if seed:
...
def main() -> None:
    ...
    all_frame_rows = read_csv(per_frame_csv)
    particles_by_frame = group_particles(particle_rows)
    per_frame_rows = frame_rows_by_index(all_frame_rows, run_id=args.run_id)
    scenario_paths = scenario_path_counts(all_frame_rows)
