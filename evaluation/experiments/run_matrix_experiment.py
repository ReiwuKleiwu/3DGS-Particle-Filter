...
def parse_particle_counts(raw_mode: dict, *, mode: str) -> list[int]:
    if "particle_count" in raw_mode and "particle_counts" in raw_mode:
        raise ValueError(f"modes.{mode} must not define both particle_count and particle_counts")
    if "particle_counts" in raw_mode:
        raw_counts = raw_mode["particle_counts"]
        if not isinstance(raw_counts, list) or not raw_counts:
            raise ValueError(f"modes.{mode}.particle_counts must be a non-empty list")
        particle_counts = [int(count) for count in raw_counts]
    else:
        particle_counts = [int(raw_mode.get("particle_count", 500 if mode == "local" else 2000))]

    invalid_counts = [count for count in particle_counts if count <= 0]
    if invalid_counts:
        raise ValueError(f"modes.{mode}.particle_counts must contain only positive values: {invalid_counts}")
    if len(set(particle_counts)) != len(particle_counts):
        raise ValueError(f"modes.{mode}.particle_counts contains duplicate values")
...
def parse_modes(raw_modes: dict) -> list[ModeSpec]:
    ...
    for mode_name, raw_mode in raw_modes.items():
        ...
        particle_counts = parse_particle_counts(raw_mode, mode=mode)
        if mode == "local":
...
def run_label(
    *,
    ...
    return f"{scenario_id}/{path_id} {splat_id} {mode.mode}{prior} particles={mode.particle_count} seed={seed}"


def particle_count_suffix_keys(modes: list[ModeSpec]) -> set[tuple[str, int]]:
    counts_by_mode_case: dict[tuple[str, int], set[int]] = defaultdict(set)
    for mode in modes:
        counts_by_mode_case[(mode.mode, mode.prior_case_index)].add(mode.particle_count)
    return {key for key, counts in counts_by_mode_case.items() if len(counts) > 1}


def run_id_particle_suffix(mode: ModeSpec, suffix_keys: set[tuple[str, int]]) -> str:
    if (mode.mode, mode.prior_case_index) not in suffix_keys:
        return ""
    return f"__particles{mode.particle_count}"


def emit_status(console, message: str, *, style: str | None = None) -> None:
...
def build_run_id(*, scenario_id: str, path_id: str, splat_id: str, mode: ModeSpec, suffix_keys: set[tuple[str, int]], seed: int) -> str:
    prior_suffix = f"__prior{mode.prior_case_index}" if mode.mode == "local" else ""
    particle_suffix = run_id_particle_suffix(mode, suffix_keys)
    return f"{scenario_id}__{path_id}__{splat_id}__{mode.mode}{prior_suffix}{particle_suffix}__seed{seed}"
    ...
    *,
    ...
    try:
        for frame_index, frame in enumerate(sampled_frames):
            ...
            best_metric_value = float(step_result.best_score)
            median_metric_value = float(step_result.median_score)
            total_frame_ms_values.append(total_frame_ms)
            render_ms_values.append(render_ms)
            best_metric_values.append(best_metric_value)
            median_metric_values.append(median_metric_value)
            ...
            frame_rows.append(
                {
                    ...
                    **errors,
                    ...
                    "median_metric_value": median_metric_value,
    ...
    summary = {
        ...
        "mean_best_metric_value": float(np.mean(best_metric_values)),
        "median_best_metric_value": float(np.median(best_metric_values)),
        "p95_best_metric_value": percentile(best_metric_values, 95),
        "final_best_metric_value": best_metric_values[-1],
        "mean_median_metric_value": float(np.mean(median_metric_values)),
        "p95_median_metric_value": percentile(median_metric_values, 95),
...
def run_matrix(args: argparse.Namespace, matrix_path: Path, *, run_name_override: str | None = None) -> None:
    matrix_path = matrix_path.resolve()
    matrix = load_matrix(matrix_path)
    matrix_dir = matrix_path.parent

    raw_experiment = matrix.get("experiment", {})
    run_name = run_name_override or raw_experiment.get("name") or datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    per_frame_rows: list[dict] = read_csv_if_exists(per_frame_path)
    per_run_rows: list[dict] = read_csv_if_exists(per_run_path)
    completed_run_ids = {row["run_id"] for row in per_run_rows if row.get("run_id")}
    ...
    particle_suffix_keys = particle_count_suffix_keys(modes)
    planned_run_ids = {
        build_run_id(
            ...
            splat_id=splat.splat_id,
            mode=mode,
            suffix_keys=particle_suffix_keys,
            seed=seed,
        )
        for path_spec in path_specs
    ...
    completed_planned_count = len(planned_run_ids & completed_run_ids)
    existing_unplanned_count = len(completed_run_ids - planned_run_ids)
    print_study_summary(
        console=console,
        run_name=run_name,
    ...
    if per_run_path.is_file():
        emit_status(
            console,
            f"Resume state found: {per_run_path}",
            style="yellow",
        )
        emit_status(
            console,
            f"Resume detected: {completed_planned_count}/{total_runs} planned runs already complete; "
            f"{total_runs - completed_planned_count} remaining.",
            style="yellow" if completed_planned_count else "red",
        )
        if existing_unplanned_count:
            emit_status(
                console,
                f"Resume note: {existing_unplanned_count} existing run IDs do not belong to this matrix and will be ignored.",
                style="yellow",
            )
    else:
        emit_status(
            console,
            f"No resume state found at {per_run_path}; starting a new result set.",
        )
    ...
    with progress if progress is not None else nullcontext():
        overall_task_id = (
            progress.add_task("overall runs", total=total_runs, completed=completed_planned_count)
            ...
            for splat in splats_by_csv[path_spec.splat_csv]:
                ...
                for mode in modes:
                    for seed in seeds:
                        ...
                        run_jobs.append((run_counter, mode, seed, run_id, label))

                pending_run_jobs = [job for job in run_jobs if job[3] not in completed_run_ids]
                if not pending_run_jobs:
                    if console is not None:
                        console.print(
                            f"[green]skip[/green] {path_spec.scenario_id}/{path_spec.path_id} "
                            f"splat={splat.splat_id}: all runs complete"
                        )
                    else:
                        print(
                            f"Skip {path_spec.scenario_id}/{path_spec.path_id} splat={splat.splat_id}: all runs complete",
                            flush=True,
                        )
                    continue
                ...
                for run_number, mode, seed, run_id, label in pending_run_jobs:
                    global_rng = random.Random(seed)
                    global_sampler = lambda rng=global_rng: free_space_sampler.sample_pose(rng=rng)
                    if console is not None:
                        console.print(f"[bold]Run {run_number}/{total_runs}[/bold] {label}")
                    else:
                        print(f"Run {run_number}/{total_runs}: {run_id}", flush=True)
                    ...
                    write_csv(per_frame_path, per_frame_rows)
...
def main() -> None:
    ...
    if len(matrix_paths) > 1 and args.run_name is not None:
        raise ValueError("--run-name can only be used with a single --matrix. Multi-matrix runs use each matrix experiment.name.")

    for matrix_index, matrix_path in enumerate(matrix_paths, start=1):
        if len(matrix_paths) > 1:
            print(f"Starting matrix {matrix_index}/{len(matrix_paths)}: {matrix_path}", flush=True)
        run_matrix(args, matrix_path, run_name_override=args.run_name)
