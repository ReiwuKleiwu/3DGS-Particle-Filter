"""Propagates particles using odometry deltas and sampled motion noise."""
    ...
    def predict(self, particles: list[Particle], odometry_delta: OdometryDelta) -> list[Particle]:
        ...
        for particle in particles:
            ...
            predicted_x = (
                ...
                + self._rng.gauss(0.0, self._noise_x)
            )
            predicted_y = (
                particle.pose.y
                + math.sin(yaw) * odometry_delta.forward_meters
                + math.cos(yaw) * odometry_delta.lateral_meters
                + self._rng.gauss(0.0, self._noise_y)
            )
            predicted_yaw = wrap_angle(yaw + odometry_delta.yaw_radians + self._rng.gauss(0.0, self._noise_yaw))
