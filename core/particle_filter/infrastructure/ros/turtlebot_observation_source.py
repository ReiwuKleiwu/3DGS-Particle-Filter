"""ROS node that gathers the latest camera, odometry, and TF state as observations."""
...
from core.particle_filter.infrastructure.ros.image_conversion import ros_image_to_rgb_array
...
from core.particle_filter.infrastructure.ros.tf_helpers import pose_measurement_from_transform, stamp_to_tuple
...
class TurtleBotObservationSource(Node):
    ...
    def _latest_amcl_pose(self):
        if self._amcl_pose_message is None:
            return None
        measurement = pose_measurement_from_transform(self._amcl_pose_message.pose.pose)
